...
class PagSeguro {
    ...
    private $appid = "app2930171537";
    private $appkey = "315D84288C8CD15004E8CFA638550446";
    private $email = "gravadorpub@gmail.com";
    private $token_sandbox = "160084195F4D4407857907BB79C4C563";
    private $token_oficial = "TOKEN DO PAGSEGURO";
    private $url_retorno = "https://gravadorpub.com.br/response.php";
    //URL OFICIAL
    ...
    private $url_notificacao = 'https://ws.sandbox.pagseguro.uol.com.br/v3/transactions/notifications/';
    private $url_transactions = 'https://ws.sandbox.pagseguro.uol.com.br/v3/transactions/';
    ...
    public function executeNotification($POST) {
        $url = $this->url_notificacao . $POST['notificationCode'];
    ...
    public function getStatusByCode($code) {
        $url = $this->url_transactions . $code;
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    ...
    public function getStatusByReference($reference) {
        ...
        if (isset($transaction_obj->transactions->transaction->status)) {
            $retorno = intval($transaction_obj->transactions->transaction->status);
            return $retorno;
        } else {
            return NULL;
        }
